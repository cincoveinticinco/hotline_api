  ...
  def self.reportAnswers()
  	Answer.select(" 
		...
		(case 
			...
			when questions.q_type_id = 2 and answers.a_yes_or_no = true then 'Yes' 
			when questions.q_type_id in (2,3)  and answers.a_yes_or_no = false then 'No'
			when questions.q_type_id = 3 and answers.a_yes_or_no = true then concat( 'Yes, ',  answers.a_txt) 
